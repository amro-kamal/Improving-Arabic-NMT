...
def log_data_info(train_data1: Dataset, train_data2: Dataset, valid_data: Dataset, test_data: Dataset,
    ...
    logger.info("Data set sizes: \n\ttask1 train %d, \n\ttask2 train %d, \n\tvalid %d,\n\ttest %d",
                len(train_data1), len(train_data2), len(valid_data),
                len(test_data) if test_data is not None else 0)

    logger.info("Task 1 First training example:\n\t[SRC] %s\n\t[TRG] %s",
                " ".join(vars(train_data1[0])['src']),
                " ".join(vars(train_data1[0])['trg']))
    logger.info("Task 2 First training example:\n\t[SRC] %s\n\t[TRG] %s",
                " ".join(vars(train_data2[0])['src']),
                " ".join(vars(train_data2[0])['trg']))
    logger.info(
