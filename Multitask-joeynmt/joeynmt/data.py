...
def load_data(data_cfg: dict, datasets: list = None)\
        -> (Dataset, Dataset, Dataset, Optional[Dataset], Vocabulary, Vocabulary):
    """
    ...
    src_lang1 = data_cfg["src1"]
    trg_lang1 = data_cfg["trg1"]
    src_lang2 = data_cfg["src2"]
    trg_lang2 = data_cfg["trg2"]
    train_path1 = data_cfg.get("train1", None)
    train_path2 = data_cfg.get("train2", None)

    dev_path = data_cfg.get("dev", None)
    test_path = data_cfg.get("test", None)

    if train_path1 is None and train_path2 is None and  dev_path is None and test_path is None:
    ...
    if "train" in datasets and train_path1 is not None:
        ...
        if random_train_subset > -1:
            ...
            keep_ratio = random_train_subset / len(train_data1)
            keep, _ = train_data1.split(
...
##############################################################################
    ...
    assert (train_data1 is not None) or (train_data2 is not None) or (src_vocab_file is not None)
    assert (train_data1 is not None) or (train_data2 is not None) or (trg_vocab_file is not None)

    #if the voc exists at the voc_file, we will load it directly, otherwise we will create the voc from the provided tokenized dataset
    logger.info("Building vocabulary...")
    src_vocab = build_vocab(field="src", min_freq=src_min_freq,
                            max_size=src_max_size,
                            dataset=[train_data1 , train_data2] , vocab_file=src_vocab_file)
    trg_vocab = build_vocab(field="trg", min_freq=trg_min_freq,
                            max_size=trg_max_size,
                            dataset=[train_data1] , vocab_file=trg_vocab_file)
##############################################################################
#Val data
##############################################################################
    dev_data = None
...
##############################################################################
    ...
    return train_data1, train_data2, dev_data, test_data, src_vocab, trg_vocab
